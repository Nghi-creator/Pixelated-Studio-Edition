-- Add normalized catalog genres and expose genre/SPDX filters through the
-- rights-cleared public catalog RPC. Existing games remain visible as `other`.

ALTER TABLE public.games
  ADD COLUMN IF NOT EXISTS genre_slug text NOT NULL DEFAULT 'other';

ALTER TABLE public.games
  DROP CONSTRAINT IF EXISTS games_genre_slug_check;

ALTER TABLE public.games
  ADD CONSTRAINT games_genre_slug_check CHECK (
    genre_slug IN (
      'action',
      'adventure',
      'arcade',
...
CREATE INDEX IF NOT EXISTS games_published_genre_idx
  ON public.games (genre_slug, lower(title))
  WHERE publication_status = 'published';

DROP FUNCTION IF EXISTS public.published_catalog_games(uuid, integer, text, text);
...
AS $$
  ...
  FROM public.games
  JOIN eligible_games ON eligible_games.id = games.id
  ORDER BY
