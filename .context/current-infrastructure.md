...
4. Electron runs a detached container named `pixelated-node` with `-p 127.0.0.1:8080:8080`, publishing the engine only to host loopback.
5. On stop/window close, Electron removes `pixelated-node`.
...
- Local engine still trusts request headers and Socket.IO events because it is assumed to run on the user's machine, but the Docker port is now published only to host loopback and the engine has restricted CORS.
