...
- Allowed origins can be configured with `PIXELATED_ALLOWED_ORIGINS`.
...
- Pairing-token enforcement is not implemented yet because the hosted React app still needs a UX path to receive or store the token generated by the local Electron app.
...
The local engine assumes that `localhost` plus an `X-User-Id` header is enough. Locally, another page or local process could send requests to `localhost:8080`.

Remaining suggested improvements:

- Generate a per-engine pairing token in Electron.
- Require the token for HTTP and Socket.IO.
...
1. Add local engine pairing token generated by Electron and required by Express/Socket.IO.
2. Add `/health` endpoint and improve Electron startup state.
3. Clean repo ignores for generated files.
4. Refactor `useWebRTC` into smaller session boot, signaling, peer connection, and input modules.
5. Add cloud ROM URL allowlisting, max download size, timeout, and temp-file cleanup.
